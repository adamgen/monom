...
`monomd pack <word...>` SHALL take one or more space-separated command tokens as CLI args, join them with `/` internally to produce a relative file path, resolve it against the project root (discovered via the same algorithm as `monomd root`), and print the resulting absolute path to stdout. The file MUST exist and MUST be executable; otherwise `monomd pack` SHALL exit non-zero with an error message on stderr.
...
- **WHEN** the project root is `/home/user/myproject` and pack is invoked as `monomd pack category1 sub_command1`
...
- **WHEN** the project root is `/home/user/myproject` and pack is invoked as `monomd pack command1`
- **THEN** stdout is `"/home/user/myproject/command1"` and exit code is 0

#### Scenario: Nested input is joined with slashes
- **WHEN** the project root is `/home/user/myproject` and pack is invoked as `monomd pack infra cloud deploy`
...
`monomd pack` SHALL discover the project root using the same algorithm as `monomd root`: if `$MONOM_PROJECT_ROOT` is set and points to a directory containing an executable `monom` file, use it; otherwise, walk up from `$PWD` looking for a directory containing an executable `monom` file. If no project root can be discovered, `monomd pack` SHALL exit non-zero with an error message on stderr.

#### Scenario: MONOM_PROJECT_ROOT is honored when valid
...
#### Scenario: Walk from PWD when MONOM_PROJECT_ROOT is unset
- **WHEN** `$MONOM_PROJECT_ROOT` is unset and a parent directory of `$PWD` contains an executable `monom` file
- **THEN** pack discovers that directory as the root

#### Scenario: No project root found causes non-zero exit
- **WHEN** `$MONOM_PROJECT_ROOT` is unset and no ancestor of `$PWD` contains an executable `monom` file
- **THEN** an error message is printed to stderr and exit code is non-zero
