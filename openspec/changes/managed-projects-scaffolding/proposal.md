...
monom needs authoring-time tooling — commands like `mnmd init` and `mnmd new command` — backed by strong defaults that get a user from nothing to a working CLI in one command.
...
- Add `monom.yaml` as an alternative project marker. For managed projects, `mnmd` handles discovery and resolution internally (fewer subprocess roundtrips).
- Add scaffolding subcommands to `mnmd`: `init`, `new command`, potentially `new project`.
...
### New Capabilities
- `managed-projects`: The managed project model — `monom.yaml` as declarative config, mnmd-internal discovery and resolution, terminology, and the constitutional principle.
- `scaffolding`: Authoring-time commands (`mnmd init`, `mnmd new command`) that generate project structure and command files with strong defaults.
...
- `architecture.md` — new mnmd subcommands, monom.yaml format, alternative discovery path
- `mnmd` binary — new subcommands (init, new), YAML parsing, internal discovery/resolution for managed projects
- `mnmd root` — needs to discover both `monom` (executable) and `monom.yaml` as project markers
- Shell bindings — need to detect managed vs custom and adjust data flow (managed: single mnmd call; custom: pipe through user config)
