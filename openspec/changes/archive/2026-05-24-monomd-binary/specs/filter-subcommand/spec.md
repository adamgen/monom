...
`monomd filter [word...]` SHALL read newline-delimited command paths from stdin (slash-delimited, e.g. `category1/sub_command1`) and accept zero or more word arguments (space-separated tokens the user has typed, e.g. `category1` `sub`). It SHALL join the word arguments with `/` internally to produce a prefix, then match that prefix against the stdin paths to determine next-level completions. No word arguments SHALL return all top-level tokens.
...
`monomd filter` SHALL silently skip any stdin line that contains a space character in any path segment. Those paths SHALL be excluded from the output as if they were not present. No error SHALL be printed and exit code SHALL remain 0. Use `monomd check` to surface these problems explicitly.
...
`monomd filter` is invoked during interactive Tab completion. A non-zero exit code or noisy stderr output would degrade the user experience mid-typing. `monomd filter` SHALL always exit with code 0, regardless of input validity, stdin read errors, or any other failure. On any internal error, it SHALL produce empty stdout and exit 0. Diagnostics belong in `monomd check`, not here.
