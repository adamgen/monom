...
The Makefile SHALL provide a `check` target that, in order: builds the binary, runs Go tests, runs all shUnit2 e2e suites under `tests/`, and runs shellcheck on all shell files under `src/` and `tests/`. `check` SHALL declare `build` as a prerequisite so the binary is always fresh before e2e tests run.
...
The Makefile SHALL provide a `lint` target that runs `shellcheck` on all shell files under `src/` and `tests/`. It SHALL respect `.shellcheckrc` at the repo root.
...
All Makefile targets (`help`, `build`, `test`, `check`, `lint`, `clean`) SHALL be declared `.PHONY` so they always run regardless of files with matching names.
