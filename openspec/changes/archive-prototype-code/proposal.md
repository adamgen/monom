...
- `archive-prototype-code`: A single atomic move of all prototype-era code into `_archive/`, preserving the files for reference while clearing the working root for clean implementation of the `mnmd` architecture.
...
- `go.mod` — stays at root (it will be updated to match the new Go source structure when `mnmd` is implemented)
