...
`monom_args [modifiers...] <name> ... -- <raw args...>` SHALL parse the declaration, call `mnmd args` for each declared flag, and set a variable with the flag's name in the caller's scope.
...
Modifiers (`--short`, `--boolean`) placed before a flag name SHALL apply to that specific flag, matching the `mnmd args` modifier syntax.
...
The `--` token SHALL separate the flag declarations (left side) from the raw argument list (right side). The raw args are passed to every `mnmd args` invocation.
